...
2. For each open PR found, call `/babysit-pr <url>` using the Skill tool, passing the PR URL.
   Run these in parallel when possible (use agents). Each parallel `/babysit-pr` must operate in its own git worktree (see `claude/git-checkouts.md` → Worktrees for every edit) so the jobs don't collide on a single checkout.
